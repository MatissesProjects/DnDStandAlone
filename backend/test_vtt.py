...
def test_websocket_subtle_roll_only_to_gm():
    # This is a bit complex to test with TestClient as it doesn't easily support multiple concurrent WS
    # But we can verify the GM sees their own subtle roll
    with client.websocket_connect("/ws/ROOM1/GM1?role=gm") as websocket:
        websocket.send_text(json.dumps({"isSubtle": True, "result": 20, "die": "d20"}))
        data = websocket.receive_text()
        assert "20" in data
