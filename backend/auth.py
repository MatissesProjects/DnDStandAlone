...
async def callback(code: str, db: Session = Depends(get_db)):
    ...
    async with httpx.AsyncClient() as client:
        ...
        user = db.query(models.User).filter(models.User.discord_id == discord_id).first()
        if not user:
            # First user to login could be GM by default, or we can have a list of GMs
        ...
        # Redirect back to frontend with the token (adjust for production)
        # For development, we'll return it as JSON
        return {
