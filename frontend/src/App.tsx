      ...
      <div className="flex items-center justify-center h-screen bg-gray-950 text-white font-sans text-center">
        <div className="space-y-8 p-12 bg-gray-900 rounded-[3rem] border border-gray-800 shadow-2xl relative overflow-hidden">
          ...
          <div className="flex flex-col gap-4">
            ...
            <button onClick={() => { fetch('http://localhost:8000/auth/guest').then(res => res.json()).then(data => { login(data.token, data.user); }); }} className="px-10 py-4 bg-gray-800 hover:bg-gray-700 text-gray-400 hover:text-white rounded-2xl font-black uppercase tracking-widest transition-all border border-gray-700 active:scale-95">Join as Guest</button>
          </div>
        </div>
      </div>
    );
    ...
    <div className={`flex w-screen h-screen bg-gray-950 text-white font-sans overflow-hidden select-none transition-all duration-300 ${(vfxRoll?.isCrit || vfxRoll?.isFail) ? 'animate-big-shake' : vfxRoll ? 'animate-shake' : ''}`}>
      ...
      {selectedEntity && (
        <NPCDetailCard 
          ...
          onUpdateStats={async (id, upd) => {
            ...
            if (res.ok) { fetchEntities(activeLocation?.id || 0); }
          }} 
          ...
          onRoll={(name, lbl, bonus) => {
            ...
            if (activeCampaign && token) fetch(`http://localhost:8000/campaigns/${activeCampaign.id}/history`, { method: 'POST', headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ event_type: 'dice_roll', content: `${name} rolled d20 (${lbl}): ${res + (bonus||0)}${isSubtleMode ? ' (Subtle)' : ''}`, campaign_id: activeCampaign.id, is_private: isSubtleMode }) }).then(r => r.json()).then(saved => sendMessage(JSON.stringify({ id: saved.id.toString(), type: 'roll', die: 'd20', result: res + (bonus||0), timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), isSubtle: isSubtleMode, user: name, senderId: clientId })));
          }} 
        />
      )}
      ...
      {rightSidebarOpen && (
        <GMToolbox 
          ...
          onManifestEntity={async () => { if (!generatedEnemy || !token || !activeLocation) return; const res = await fetch(`http://localhost:8000/entities`, { method: 'POST', headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ name: generatedEnemy.name, location_id: activeLocation.id, stats: generatedEnemy.stats, backstory: generatedEnemy.backstory }) }); if (res.ok) { setGeneratedEnemy(null); fetchEntities(activeLocation.id); } }} 
