...
interface WorldDashboardProps {
  campaignId: number;
  onClose: () => void;
  onSetActive: (location: Location, sceneId: string) => void;
  activeLocationId?: number;
  currentTargetScene: string;
}

const WorldDashboard: React.FC<WorldDashboardProps> = ({ campaignId, onClose, onSetActive, activeLocationId, currentTargetScene }) => {
  const { token } = useAuth();
  const [locations, setLocations] = useState<Location[]>([]);
  const [entities, setEntities] = useState<Entity[]>([]);
  const [activeTab, setActiveTab] = useState<'locations' | 'entities'>('locations');
  const [projectionTarget, setProjectionTarget] = useState(currentTargetScene);
                      ...
                      <div key={loc.id} className={`p-6 rounded-3xl border transition-all flex justify-between items-start group ${activeLocationId === loc.id ? 'bg-indigo-900/10 border-indigo-500/40 shadow-xl' : 'bg-gray-900 border-gray-800 hover:border-gray-700'}`}>
                        ...
                        <div className="flex gap-2 shrink-0 items-center">
                          <select 
                            ...
                            className="bg-gray-950 border border-gray-800 rounded-lg px-2 py-1.5 text-[8px] font-black uppercase text-indigo-400 focus:outline-none"
                          >
                            <option value="main">To Main</option>
                            <option value="alpha">To Alpha</option>
                            <option value="beta">To Beta</option>
                            <option value="global">To Global</option>
                          </select>
                          <button onClick={() => onSetActive(loc, projectionTarget)} className="bg-gray-800 hover:bg-indigo-600 text-[9px] font-black uppercase px-4 py-2 rounded-xl border border-gray-700 transition-all">Project</button>
